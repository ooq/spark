...
private[spark] class UnserializedCopyObjectWriter(manager: BlockManager,
  ...
  def open() {
    println("opening nocopy writer " + ser)
    initialized = true
  }
  ...
  def write(key: Any, value: Any) {
    ...
    if (value.getClass() == classOf[String]) {
      val newValue = new String(value.asInstanceOf[String])
